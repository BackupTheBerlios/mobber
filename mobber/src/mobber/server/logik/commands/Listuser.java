...
public class Listuser extends Command
{
	...
	Element listuser = new Element("listuser");
	root.addContent(listuser);

	for (int i = 0; i < ClientList.users.size(); i++){
	    String name = ((User)ClientList.users.get(i)).getName();	    
	    Element user = new Element("user");
	    user.setText(name);
	    listuser.addContent(user);
	}
